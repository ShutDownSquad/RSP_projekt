...
<body>
    <header>
        ...
        <div id="user-info">
            <?php
            if (isset($_SESSION['jmeno']) && isset($_SESSION['role']) && isset($_SESSION['image_path'])) {
            $image_path = $row['image_path'] ?? 'default_image.jpg';
            } else {
            echo '<span>Welcome, Guest</span>';
            }
            ?>
            <img id="user-icon" src="<?php echo $image_path; ?>" alt="User Avatar" style="width: 30px; height: 30px; margin-left: 100px; cursor: pointer; border-radius: 100px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);">
        </div>
